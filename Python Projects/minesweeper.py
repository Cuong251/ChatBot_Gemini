import random
import re
# lets create a board object to represent this minesweeper game
...
class Board:
    ...
    def assign_values_to_board(self):
        ...
        # represents how many neighboring bombs there are, we can precompute these and it'll save
        # effort checking what's around the board later on :)
        for r in range(self.dim_size):
            for c in range(self.dim_size):
                if self.board[r][c] == '*':
                    # if this is alreadt a bomb, we don't want to calculate anything
                    continue
                self.board[r][c] = self.get_num_neighboring_bombs(r,c)
    def get_num_neighboring_bombs(self, row, col):
        # let's interate through each of the neighboring positions and sum number of bombs
    ...
    def dig(self, row, col):
        ...
        # dis at location with no neighboring bombs -> recursively dig neighbors!
        ...
        # if put initial dig didn't hit a bomb, we shouldn't hit a bomb here
        return True
...
def __str__(self):
    ...
    # Build the column indices header
    string_rep = "   " + "  ".join(f"{i}" for i in range(self.dim_size)) + "\n"

    for i in range(self.dim_size):
        row_repr = f"{i} |" + "|".join(visible_board[i]) + "|\n"
        string_rep += row_repr
